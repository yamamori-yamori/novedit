...
- 日本語長編小説を前提に設計されていて、10万文字超えていても快適に動作します。
...
- undoは通常のテキスト編集では制限がありませんが、全文を対象とする動作（ツールや全て置換、新規、挿入など）は10回までです。  

## キー操作リスト

### ショートカットキー
- Ctrl/Cmd+Z          Undo（戻す）。── 本文まるごとの差し替え（新規・読み込みなど）は自前で戻し、ふだんの入力はブラウザ任せ
...
- Ctrl/Cmd+A                               全選択(編集エリアのみ)
- Ctrl/Cmd+Shift+Z / Ctrl+Y                やり直し（Redo、ふだんの入力のみ。全文に対する操作にはRedoはない)
