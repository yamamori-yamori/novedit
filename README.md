...
* 検索でミニマップ上に全件ハイライト表示や、左右差分モードなど、一覧性に優れています。
* 作者HPにWEB版もあります。
...
　参照テキストタブの他に、右ペインで編集しているテキストの初期状態のタブもありますので、間違えて消してしまった時などにコピーで戻すことができます。なお、左右ペインの区切りはマウスで動かせます。
4. 真ん中上に「差分」ボタンがあり、それを押すと左右差分比較モードになります。
5. スクロールホイールで高速スクロールできます。また両脇のミニマップをマウスドラッグでも高速スクロールできます。
6. ctrl-Zか右上の「戻す」でundo可能です。さらに編集したテキストはブラウザに自動保存されています。ブラウザを閉じても次回開いたときに続きを再開できます。
...
* `\\\\\\\\\\\\\\\*\\\\\\\\\\\\\\\*`や`\\\\\\\\\\\\\\\*\\\\\\\\\\\\\\\*\\\\\\\\\\\\\\\*`で囲んだ単語は本文に赤と青で強調表示できます。キーワードをプロットに記載するときなど便利。
* 小説家になろうやカクヨムで使えるルビ用の《》も紫で強調表示でき、左ペインにはルビとして表示されます。
...
中央上の「差分」ボタンを押すと、左右のペインを行ごとに比較し違いのあるテキストに色付けをして表示するモードになります。  
prev↑/next↓で前次の差分にオレンジのカーソルが移動します。
...
* 範囲設定をしてから、右ペイン・検索窓の右の「ルビ」を押すと小説家になろうやカクヨムで使えるルビを入力できます。
* 例えば「漢字」を選択して「ルビ」を押すと「|漢字《》」となります。

<img src="ルビ.png" width="50%">  
...
* 中央上に「差分」ボタンを配置し、ここから差分モードに移行できる。この時は行ごとの編集となる。
...
* 全く違う長文比較で差分モードが重くなる現象を改善。
* 復元から上書き保存時になるべく元のファイルに保存する。
...
* PCでのPWAモード（ブラウザアプリモード）で拡張子紐づけしている場合、二つ目以降のファイルクリックで新ウィンドウでなく新タブにファイルを入れる。
...
* ver.1.4.9で入れたPCでのPWAモードで、ファイルを追加するタブを左側に変更(右タブが編集中の場合があるため）
* 長文での動きを高速化した。
...
* 見出しの強調文字の中に《》を「ルビ」として追加しました。
* 上記ルビがONであれば、左ペインには小説家になろう・カクヨム形式の「|漢字《かんじ》」というルビが表示されます。
* 右ペイン検索窓の右に「ルビ」ボタンを付けました。10文字以内の文字を選択して「ルビ」を押すとタグが挿入されます。
...
* 長文でDELキーのレスポンスが遅くなるのを改善。
* 長文で検索が大量にヒットした場合、ルビが大量にある場合の軽量化。
